...
public class Drag : MonoBehaviour
{
    ...
    void Update()
    {
        //Debug.Log("bolean orange" + MenuScript.orangeLevel);
    ...
    private void OnTriggerEnter(Collider other)
    {
        ...
        if(MenuScript.greenLevel)
        {
            ...
            //BLUE-GREEN
            if (sphere.gameObject.tag == "blue" && other.gameObject.tag == "green" || sphere.gameObject.tag == "green" && other.gameObject.tag == "blue")
            {
                foreach (var x in materials)
                {
                    if (x.name == "blueGreen")
                    {
                        sphere.GetComponent<Renderer>().material = x;
                        sphere.gameObject.tag = "blueGreen";
                        Debug.Log("Convertir en blueGreen.");
                    }
                }
                joinAudio.Play(0);
                StartCoroutine(tweenCoroutine());
